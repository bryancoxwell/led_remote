...
async fn handle_finish(
    ...
    match handler.session {
        ...
        Some(ref mut session) => {
            ...
            // led_remote vendor patch: upstream calls SigningKey::from_bytes
            // here, which interprets pairing.public_key as a *secret key seed*
            // and derives an unrelated public key for verification — making
            // pair-verify M3 always fail with Authentication. The controller's
            // public key must be loaded as a VerifyingKey instead.
